    ...
    <div className="bg-[#141210]/80 border border-[#C04A2A]/20 p-8 lg:p-10 rounded-sm text-[#F0E8D8] relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-8 gap-4">
        <div>
          ...
          <h2 className="font-display text-3xl tracking-wide">Alla Inlägg</h2>
        </div>
        <a
          href="/admin/new"
      ...
      <div className="space-y-4">
        {filteredPosts.map((post) => (
          <div key={post.id} className="relative group p-6 bg-[#1A1816]/50 border border-[#C04A2A]/20 hover:border-[#C04A2A]/50 rounded-sm transition-all hover:bg-[#1C1A18]">
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                <a href={`/admin/edit/${post.id}`} className="font-display text-xl tracking-wide text-[#F0E8D8] hover:text-[#C04A2A] transition-colors mb-1 block">{post.title}</a>
                <p className="text-xs text-[#F0E8D8]/40 font-mono tracking-tight">/{post.slug}</p>
              </div>
              <div className="flex items-center gap-4">
                ...
                <a href={`/admin/edit/${post.id}`} className="text-[#F0E8D8]/50 hover:text-[#F0E8D8] transition-colors text-[10px] uppercase tracking-wider font-medium ml-2">Redigera</a>
                <button onClick={() => handleDelete(post.id)} className="text-red-500/60 hover:text-red-400 transition-colors text-[10px] uppercase tracking-wider font-medium ml-2">Radera</button>
              </div>
            </div>
          </div>
        ))}
