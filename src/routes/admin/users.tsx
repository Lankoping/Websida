        ...
        {createdUserLink && (
          <div className={`mt-6 p-4 border rounded-md ${createdUserLink.emailSent ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-amber-500/30 bg-amber-500/10'}`}>
            ...
            <div className="p-3 bg-background border border-border rounded text-sm font-mono text-muted-foreground whitespace-pre-wrap">
              ...
              {currentUser?.name || 'En administratör'} har begärt att du skapar ett Lankoping-konto.{'\n\n'}
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              ...
              {!createdUserLink.emailSent && (
                <a 
                  href={`mailto:${createdUserLink.email}?subject=Ditt Lankoping-konto&body=Hej ${createdUserLink.name}!%0D%0A%0D%0A${currentUser?.name || 'En administratör'} har begärt att du skapar ett Lankoping-konto.%0D%0A%0D%0AGå till följande länk för att skapa ditt lösenord:%0D%0A${encodeURIComponent(createdUserLink.link)}`}
